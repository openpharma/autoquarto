...
`autoquarto` is very much a work in progress, and is being developed in parallel with the [`rbqmR`](https://openpharma.github.io/rbqmR/main/) package.  It is also being used in shinyCrmPack, a Shiny front-end to the [crmPack](https://github.com/Roche/crmPack) package.  As `rbqmR` and `shinyCrmPack` evolve, so will `autoquarto`.
